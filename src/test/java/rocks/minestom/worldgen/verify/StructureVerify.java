...
/**
 * Scans a vanilla world save for structure starts matching a name filter within a
 * chunk range, compares against this library's locateStructure result, and
 * runs a block-level diff over the structure bounding box.
 *
 * <p>Usage: StructureVerify &lt;vanillaWorldDir&gt; &lt;datapackDir&gt; &lt;seed&gt;
 * &lt;minChunkX&gt; &lt;minChunkZ&gt; &lt;maxChunkX&gt; &lt;maxChunkZ&gt;
 * &lt;structureNameFilter&gt; &lt;locateStructureKey&gt;
 */
public final class StructureVerify {
    public static void main(String[] args) throws Exception {
        ...
        var nameFilter = args[7];
        var locateKey = Key.key(args[8]);

        System.out.println("=== scanning vanilla structure starts for " + nameFilter + " ===");
        var regions = new HashMap<Long, RegionFile>();
        Integer foundChunkX = null;
        Integer foundChunkZ = null;
        CompoundBinaryTag foundStart = null;
        ...
        if (foundStart == null) {
            System.out.println("No " + nameFilter + " start found in scanned chunk range ["
        ...
        var located = worldGenerator.locateStructure(locateKey,
                centerChunkX * 16, centerChunkZ * 16, Math.max(maxChunkX - minChunkX, maxChunkZ - minChunkZ) + 4);
        System.out.println("locateStructure(" + locateKey.asString() + ") around chunk " + centerChunkX + "," + centerChunkZ
                + " -> " + located);
        if (foundChunkX != null) {
            System.out.println("Expected chunk (vanilla start): " + foundChunkX + "," + foundChunkZ
                    + " -> block " + (foundChunkX * 16) + "," + (foundChunkZ * 16));
        }
        ...
        for (var childTag : foundStart.getList("Children")) {
        ...
        System.out.println(nameFilter + " bounding box: [" + minX + "," + minY + "," + minZ + "] .. ["
                + maxX + "," + maxY + "," + maxZ + "]");

        System.out.println("=== block diff over " + nameFilter + " bounding box ===");
        ...
        System.out.printf(nameFilter + " box accuracy: %d / %d (%.4f%%)%n", matched, total, 100.0 * matched / Math.max(total, 1));
